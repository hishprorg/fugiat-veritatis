# @hishprorg/fugiat-veritatis

[![npm](https://img.shields.io/npm/v/@hishprorg/fugiat-veritatis.svg)](https://npmjs.com/package/@hishprorg/fugiat-veritatis)

A JavaScript ANSI color/style management. ANSI parsing. ANSI to CSS. Small, clean, no dependencies.

```bash
npm install @hishprorg/fugiat-veritatis
```
...
```javascript
require ('@hishprorg/fugiat-veritatis').nice // .nice for unsafe String extensions
...
```javascript
import { green, inverse, bgLightCyan, underline, dim } from '@hishprorg/fugiat-veritatis'
```
```javascript
const { green, inverse, bgLightCyan, underline, dim } = require ('@hishprorg/fugiat-veritatis')
```
...
```javascript
import { @hishprorg/fugiat-veritatis, ParsedSpan } from '@hishprorg/fugiat-veritatis' // along with type definitions
```
```javascript
import @hishprorg/fugiat-veritatis from '@hishprorg/fugiat-veritatis'
```

### Nice Mode (not recommended)

```javascript
const ansi = require ('@hishprorg/fugiat-veritatis').nice
```

The `('@hishprorg/fugiat-veritatis').nice` export defines styling APIs on the `String` prototype directly. It uses an ad-hoc DSL (sort of) for infix-style string coloring. The `nice` is convenient, but not safe, avoid using it in public modules, as it alters global objects, and that might cause potential hard-to-debug compatibility issues.
...
```javascript
const { names } = require ('@hishprorg/fugiat-veritatis')
...
```javascript
const { strip } = require ('@hishprorg/fugiat-veritatis')
...
```javascript
const { isEscaped, green } = require ('@hishprorg/fugiat-veritatis')
...
```javascript
const { parse } = require ('@hishprorg/fugiat-veritatis')
...
```javascript
const ansi = require ('@hishprorg/fugiat-veritatis')
...
```javascript
const { bgGreen, red, parse } = require ('@hishprorg/fugiat-veritatis')
...
## Projects That Use `@hishprorg/fugiat-veritatis`
